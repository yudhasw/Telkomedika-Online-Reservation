...
def validate_register(data):
    ...
    # PERBAIKAN: Tambahkan (or "") untuk mencegah NoneType error di awal
    cleaned = {
        "nama": (data.get("nama") or "").strip(),
        "email": (data.get("email") or "").strip(),
        "phone" : (data.get("nomor_hp") or ""), # Jangan strip dulu disini
    ...
    # Cek Database (Optional: pindahkan ke routes jika ingin validator murni logic)
    ...
    # Syarat lain opsional (kapital/simbol) bisa diaktifkan jika perlu
    ...
    if not phone:
        ...
        # AMAN: phone pasti string, jadi bisa di-strip
        phone = phone.strip().replace(" ", "").replace("-", "")
        cleaned["phone"] = phone # Update data bersih
    ...
    # Cek apakah ada error (value tidak None)
